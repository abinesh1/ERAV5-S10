...
**Where the numbers come from.** §1–§4 and §6 are hardware-independent: the shapes, the
gradient identity, the accumulation algebra and the bit patterns come out the same
anywhere. **§5 is not** — MFU is a statement about a specific device, so it is reported
against that device's own measured ceilings, and the section names the machine every time
it quotes a number. The tables in §5 are a **CPU reference run**; open the notebook on a
GPU and its last cell prints the replacements.
